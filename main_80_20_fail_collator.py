...
class IndexingCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        inputs = {
            ...
            'input_labels': input_labels.to(device=device),
            'ids': ids,
            'pixel_values': pixel_values,
            'attention_mask': attention_mask.to(device=device),
        }

        # Process LIDAR values inside datacollator
        #lidar_values = self._prepare_lidar_values(features, device)
        #inputs['lidar_values'] = lidar_values
        
        lidar_values0 = [x['lidar_values_load'] for x in features]
        points = lidar_values0[0]['points']
        inputs['points'] = points
        
        lidar_values2  = {'batch_size': self.batch_size}
        
        #feature_dict0 = {k: [x[k] for x in features] for k in features[0].keys()}
        #feature_dict2 = {'lidar_values_load': [x['lidar_values_load'] for x in features]}
        feature_dict = {k: [x['lidar_values_load'][k] for x in features] for k in features[0]['lidar_values_load'].keys()}

	#feature_dict = {k: [x[k] for x in features] for k in features[0]['lidar_values_load'].keys()}

        points = [x['points'] for x in features]
        
        #features = features['lidar_values_load']
        ...
        for key, val in feature_dict.items():
            if key == 'points':
                ...
                padded_points = [torch.nn.functional.pad(
                    ...
                    coor.clone().detach().to(torch.float32),
                    (0, 0, 1, 0), 
                    value=i  
                ) for i, coor in enumerate(val)]
                lidar_values2[key] = torch.cat(padded_points, dim=0).to(device)
            elif key == 'lidar_values_load':
                continue
            else:
               lidar_values2[key] = val
        
        import pdb; pdb.set_trace()
        inputs['lidar_values'] = lidar_values2
...
#    import pdb; pdb.set_trace()    
    ...
    if do_eval  : 
        del train_set
        print("start eval")
        eval_log3dnet(model_dsi, eval_subset, eval_set, eval_loader, data_collator, tokenizer, cfg, checkpoint_dir, checkp_to_eval)
