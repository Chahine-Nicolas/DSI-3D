...
def main():
    ...
    if (idr_torch.rank % idr_torch.size == 0):
        ...
        logging.info('dist size: ' + str(idr_torch.size))
        logging.info('dist rank: ' + str(idr_torch.rank))
    ...
    model = torch.nn.parallel.DistributedDataParallel(
        model.to('cuda:%d' % idr_torch.local_rank),
    ...
    train_loader = make_data_loader(cfg,
                                    ...
                                    shuffle=True,
                                    dist=[idr_torch.size, idr_torch.rank])
    ...
    for epoch in range(starting_epoch, cfg.max_epoch):
        ...
        for i, batch in enumerate(train_loader, 0):
            if cfg.train_pipeline == 'LOGG3D':
                batch_st = batch[0].to('cuda:%d' % idr_torch.local_rank)
                ...
                batch_t = batch.to('cuda:%d' % idr_torch.local_rank)
        ...
        if cfg.save_model_after_epoch and (idr_torch.rank % idr_torch.size == 0):
