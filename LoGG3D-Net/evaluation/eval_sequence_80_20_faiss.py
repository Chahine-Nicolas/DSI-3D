...
def evaluate_sequence_reg(model, cfg):
    ...
    for query_idx in range(num_queries):
        ...
        seen_ids.append(query_idx)


    #data = np.stack([t.detach().cpu().numpy() for t in seen_descriptors[0]])
    data =  np.stack([seen_descriptors]).squeeze().astype('float32')
    D = 256  
    num_bits = 256
    ...
    print("len(seen_descriptors) ", len(seen_descriptors))
    ...
    for query_idx in range(num_queries):
        ...
        query = global_descriptor.astype('float32')
        ...
        ret_timer.toc()
        nearest_idx = I[0][0]
        min_dist = feat_dists[nearest_idx]
