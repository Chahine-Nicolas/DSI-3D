...
def evaluate_sequence_reg(model, cfg):
    ...
    if 'Kitti' in cfg.eval_dataset:
        ...
        if eval_seq == "22":
            is_revisit_list = revisit_json["00"] + revisit_json["02"] + revisit_json["05"] + revisit_json["06"] + revisit_json["07"]  + revisit_json["08"]
        else :
    ...
    start_time = 0 #timestamps[0]
    len_eval = 0
    
    # create the sees_descriptors database
    
    # name_list
    path = '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/lidarhd_v1/bin/'
    fname = []
    ...
    # Load the revisits dictionary
    with open(path[:-4] + "revisits3m.json") as f:
        data2 = json.load(f)
    ...
    for key, value in data2.items():
        if key not in train_set and key not in eval_set:
            train_set.append(key)
            for val in value:
                eval_set.append(val)
                
    #train_set = ['/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/lidarhd_v1/bin/LHD_FXX_0656_6860_PTS_O_LAMB93_IGN69.copc_0_0_47.bin']

    #for filename in fname: 
    for query_idx in range(len(train_set)):
        """
        if query_idx% 5 == 0:
            continue
        ...
        if query_idx < 100:
            continue
        """
        logging.info(f'seen id: {query_idx}')
         
        #import pdb; pdb.set_trace()

        filename_train = train_set[query_idx]
        #lidar_pc = input_data[0][0]  # .cpu().detach().numpy()   	
        point_cloud = np.fromfile(filename_train, dtype=np.float32)
       
        if len(point_cloud) == 0:
            continue
        lidar_pc = point_cloud.reshape(-1, 4)  
        ...
        query_pose = np.array( data[filename_train] ) # for lidar hd
        ...
        seen_ids.append(filename_train)
    ...
    #for filename in fname: 
    for query_idx in range(len(eval_set)):
        """
        query_idx += 1 
        ...
        """
        filename = '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/lidarhd_v1/bin/LHD_FXX_0656_6860_PTS_O_LAMB93_IGN69.copc_0_0_47.bin'


        filename =  '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/lidarhd_v1/bin/LHD_FXX_0656_6860_PTS_O_LAMB93_IGN69.copc_3_22_47.bin'
        point_cloud = np.fromfile(filename, dtype=np.float32)
        lidar_pc = point_cloud.reshape(-1, 4)  
        input = make_sparse_tensor(lidar_pc, cfg.voxel_size).cuda()
        output_desc, output_feats = model(input)  # .squeeze()
        global_descriptor2 = output_desc.cpu().detach().numpy()
        global_descriptor2 = np.reshape(global_descriptor2, (1, -1))

        import pdb; pdb.set_trace()
        """
        query_pose = np.array( data[filename] ) # for lidar hd
        #import pdb; pdb.set_trace()
        #query_pose = positions_database[query_idx]
        ...
        logging.info(
            f'id: {query_idx} n_id: {seen_ids[nearest_idx]} is_rev: {is_revisit} is_correct_loc: {is_correct_loc} min_dist: {min_dist} p_dist: {p_dist}')
        
        #if seen_ids[nearest_idx]% 5 == 0:
            #import pdb; pdb.set_trace()
        ...
        cfg.revisit_criteria = 3
        #cfg.not_revisit_criteria =3
        ...
        for thres_idx in range(num_thresholds):
            ...
            if(min_dist < threshold):  # Positive Prediction
                ...
                if(is_revisit == 0):
                    #if p_dist >= cfg.not_revisit_criteria:
                    num_true_negative[thres_idx] += 1
                else:
                    #elif p_dist <= cfg.revisit_criteria:
