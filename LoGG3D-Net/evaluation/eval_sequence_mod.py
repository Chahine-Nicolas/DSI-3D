...
file_tp =  open('/gpfswork/rech/dki/ujo91el/code/logg3dnet/pcd_tp'+str(1)+".txt","w")
file_fp =  open('/gpfswork/rech/dki/ujo91el/code/logg3dnet/pcd_fp'+str(1)+".txt","w")
file_tn =  open('/gpfswork/rech/dki/ujo91el/code/logg3dnet/pcd_tn'+str(1)+".txt","w")
file_fn =  open('/gpfswork/rech/dki/ujo91el/code/logg3dnet/pcd_fn'+str(1)+".txt","w")


############################################################################################
def save_tensor(xx, query_idx, sequence_path,lab) :
        file = '%06d' % query_idx
        fname =  file +'.pt'
        torch.save(xx, sequence_path + lab + "/" + fname)
        print("save at ", sequence_path + lab + "/" + fname)
        return 
############################################################################################
...
def evaluate_sequence_reg(model, cfg):
    ...
    for query_idx in range(num_queries):
        ...
        prep_timer.tic()
        lidar_pc = input_data[0][0] #.cpu().detach().numpy() # [array([[ 5.9807991e+01,  2.9970491e-02,  2.2320013e+00,  0.0000000e+00],
        ...
        output_desc_768 = output_desc.repeat(3)
        #print("output_desc ", LA.norm(output_desc.cpu().detach().numpy()))
        #print("output_desc_768 ", LA.norm(output_desc_768.cpu().detach().numpy()))
        
        output_desc_768 = output_desc_768 / LA.norm(output_desc_768.cpu().detach().numpy())
        #print("output_desc_768 ", LA.norm(output_desc_768.cpu().detach().numpy()))
        global_descriptor = output_desc_768.cpu().detach().numpy()
        global_descriptor = np.reshape(global_descriptor, (1, -1))
        """
        #import pdb; pdb.set_trace()
        ############################################################################################
        ############################################################################################
        # descriptors augmentation
        """
        output_desc_768 = output_desc.repeat(3)
        global_descriptor = output_desc_768.cpu().detach().numpy()
        global_descriptor = np.reshape(global_descriptor, (1, -1))
        ...
        """
        ############################################################################################
        # Extract from positionnal encodings
        """
        ii = 1
        d = 16
        output_desc_768 = output_desc.repeat(3)
        ...
        if ii % 2 == 0 :
            denominator = np.power(10000, 2*ii/d)
            global_descriptor = torch.sin( output_desc_768 /denominator).cpu().detach().numpy()
        else :
            denominator = np.power(10000, 2*(ii-1)/d)
            global_descriptor = torch.cos( output_desc_768 /denominator).cpu().detach().numpy()
        global_descriptor = np.reshape(global_descriptor, (1, -1))
        """
        ############################################################################################
        # norma
        """
        output_desc_768 = output_desc.repeat(3)
        post_layernorm = nn.LayerNorm(768, eps=1e-12)  
        global_descriptor = post_layernorm(output_desc_768.to(dtype=torch.float32).cpu()).detach().numpy()  
        global_descriptor = np.reshape(global_descriptor, (1, -1))
     
        
        post_layernorm = nn.LayerNorm(256, eps=1e-12)  
        global_descriptor = post_layernorm(output_desc.to(dtype=torch.float32).cpu()).detach().numpy()  
        global_descriptor = np.reshape(global_descriptor, (1, -1))
        ...
        global_descriptor = post_layernorm(output_desc_768.to(dtype=torch.float32).cpu()).detach().numpy() 
        from numpy import linalg as LA
        LA.norm(global_descriptor)
        global_descriptor = global_descriptor / LA.norm(global_descriptor)
        LA.norm(global_descriptor)
        global_descriptor = np.reshape(global_descriptor, (1, -1))
        """
        ############################################################################################
        ...
        #norm_768 = output_desc_768 / LA.norm(output_desc_768.cpu().detach().numpy())
     
        post_layernorm = nn.LayerNorm(768, eps=1e-12) 
        norm_768 = post_layernorm(output_desc_768.to(dtype=torch.float32).cpu())
      
        #norm_768 = norm_768 / LA.norm(norm_768.detach().numpy())
        #import pdb; pdb.set_trace()
        
        
        
        if ii % 2 == 0 :
            denominator = np.power(10000, ii/ddm)
            global_descriptor = torch.sin( norm_768 /denominator).detach().numpy()
        else :
            denominator = np.power(10000, (ii-1)/ddm)
            global_descriptor = torch.cos( norm_768 /denominator).detach().numpy()
        global_descriptor = np.reshape(global_descriptor, (1, -1))
        """
        ############################################################################################
        ############################################################################################
        ...
        ############################################################################################
        #save_tensor(output_desc, query_idx, sequence_path, 'logg_desc') #new
        #save_tensor(output_desc, query_idx, '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/datasets/sequences/all/', 'logg_desc') 

        sequence_path_all = '/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/datasets/sequences/all/'
        file = eval_seq +'%06d' % query_idx
        fname =  file +'.pt'
        ...
        for thres_idx in range(num_thresholds):
            ...
            if(min_dist < threshold):  # Positive Prediction
                if p_dist <= cfg.revisit_criteria:
                    ...
                    if thres_idx == 999:
                        file_tp.write("query id: "+str(query_idx)+"nearest id:"+str(nearest_idx)+" thres_idx"+str(thres_idx)+" threshold"+str(threshold)+"\n")

                elif p_dist > cfg.not_revisit_criteria:
                    num_false_positive[thres_idx] += 1
                    if thres_idx == 999:
                        file_fp.write("id: "+str(query_idx)+"nearest id:"+str(nearest_idx)+" thres_idx"+str(thres_idx)+" threshold"+str(threshold)+"\n")

            else:  # Negative Prediction
                if(is_revisit == 0):
                    num_true_negative[thres_idx] += 1
                    if thres_idx == 999:
                        file_tn.write("id: "+str(query_idx)+"nearest id:"+str(nearest_idx)+" thres_idx"+str(thres_idx)+" threshold"+str(threshold)+"\n")

                else:
                    num_false_negative[thres_idx] += 1
                    if thres_idx == 999:
                        file_fn.write("id: "+str(query_idx)+"nearest id:"+str(nearest_idx)+" thres_idx"+str(thres_idx)+" threshold"+str(threshold)+"\n")
    
    ############################################################################################
    hit = hit / num_queries
    logging.info('Hit full sequence:')
    logging.info(f'Hitscore : {round(hit,4)}')

    hit2 = hit2 / num_rev
    logging.info('Hit only revisited:')
    logging.info(f'Hitscore : {round(hit2,4)}')
    ############################################################################################
    ...
    if not save_descriptors:
        ...
        if plot_pr_curve:
            ...
            print("save_dir + '/' + eval_seq + '.png' ", save_dir + '/' + eval_seq + '.png')
    ...
    #global_descriptor == torch.load('/lustre/fsn1/worksf/projects/rech/dki/ujo91el/datas/datasets/sequences/06/logg_desc/001100.pt')
