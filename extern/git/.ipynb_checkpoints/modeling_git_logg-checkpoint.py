...
class GitLidarModel(GitPreTrainedModel):
    ...
    def forward(
        ...
        encoder_chekp: Optional[str] = None,
        ...
        if True :
            ...
            if encoder_chekp is not None:

                """
                if int(lidar_values[input_mod][0][1][:-4]) < 4541:
                    eval_seq = '00'
                    new_number =  lidar_values[input_mod][0][1][:-4] +'.bin'
                elif 4541 <= int(lidar_values[input_mod][0][1][:-4]) < 9202:
                    eval_seq = '02'
                    new_number = '%06d' % (int(lidar_values[input_mod][0][1][:-4]) - 4541 ) +'.bin'
                elif 9203 <= int(lidar_values[input_mod][0][1][:-4]) < 11964:
                    eval_seq = '05'
                    new_number = '%06d' % (int(lidar_values[input_mod][0][1][:-4]) - 9203 ) +'.bin'
                elif 11965 <= int(lidar_values[input_mod][0][1][:-4]) < 13066:
                    eval_seq = '06'
                    new_number = '%06d' % (int(lidar_values[input_mod][0][1][:-4]) - 11965 ) +'.bin'
                elif 13067 <= int(lidar_values[input_mod][0][1][:-4]) < 14168:
                    eval_seq = '07'
                    new_number = '%06d' % (int(lidar_values[input_mod][0][1][:-4]) - 13067 ) +'.bin'
                elif 14169 <= int(lidar_values[input_mod][0][1][:-4]):
                    eval_seq = '08'
                    new_number = '%06d' %( int(lidar_values[input_mod][0][1][:-4]) - 14169) +'.bin'
                """
                
                if encoder_chekp == '/lustre/fswork/projects/rech/dki/ujo91el/checkpoint/LoGG3D-NET/checkpoints/kitti_10cm_loo/2021-09-14_03-43-02_3n24h_Kitti_v10_q29_10s0_262447.pth':
                    eval_seq = '00'
                elif encoder_chekp == '/lustre/fswork/projects/rech/dki/ujo91el/checkpoint/LoGG3D-NET/checkpoints//kitti_10cm_loo/2021-09-14_05-55-20_3n24h_Kitti_v10_q29_10s2_262448.pth':
                    eval_seq = '02'
                elif encoder_chekp == '/lustre/fswork/projects/rech/dki/ujo91el/checkpoint/LoGG3D-NET/checkpoints//kitti_10cm_loo/2021-09-14_06-11-58_3n24h_Kitti_v10_q29_10s5_262449.pth':
                    eval_seq = '05'
                elif encoder_chekp == '/lustre/fswork/projects/rech/dki/ujo91el/checkpoint/LoGG3D-NET/checkpoints//kitti_10cm_loo/2021-09-14_06-43-47_3n24h_Kitti_v10_q29_10s6_262450.pth':
                    eval_seq = '06'
                elif encoder_chekp == '/lustre/fswork/projects/rech/dki/ujo91el/checkpoint/LoGG3D-NET/checkpoints//kitti_10cm_loo/2021-09-14_08-34-46_3n24h_Kitti_v10_q29_10s7_262451.pth':
                    eval_seq = '07'
                elif encoder_chekp == '/lustre/fswork/projects/rech/dki/ujo91el/checkpoint/LoGG3D-NET/checkpoints//kitti_10cm_loo/2021-09-14_20-28-22_3n24h_Kitti_v10_q29_10s8_263169.pth':
                    eval_seq = '08'
                ...
                try:
                  ...
                  lidar_pc = lidar_values['points'].cpu().numpy().astype(dtype="float32") # en cas de combinaison seq + query introuvable

                def call_logg3dnet(encoder_chekp, lidar_pc):
                    #cfg = get_config_eval()
                    voxel_size = 0.1
                    # Get model
                    model = get_pipeline('LOGG3D')
                    
                    save_path = encoder_chekp
                    ...
                    lidar_pc_augmented = lidar_pc
                    if random_rotation:
                        lidar_pc_augmented = random_rotate(lidar_pc_augmented)
                    if random_occlusion:
                        lidar_pc_augmented = occlude_scan(lidar_pc_augmented)
                    if random_scale and random.random() < 0.95:
                        scale = min_scale + \
                            (max_scale - min_scale) * random.random()
                        lidar_pc_augmented = scale * lidar_pc_augmented
            
                    input_augmented = make_sparse_tensor(lidar_pc_augmented, voxel_size).cuda()
                    output_desc2, output_feats2 = model(input_augmented)  # .squeeze()
                    #output_feats2 = output_feats2[0]
                    #global_descriptor2 = output_desc2.cpu().detach().numpy()
                    return output_desc2

            output_desc2 = call_logg3dnet(encoder_chekp, lidar_pc)
            #lidar_pc3 = lidar_values['points'].cpu().numpy().astype(dtype="float32")
            #output_desc3 = call_logg3dnet(encoder_chekp, lidar_pc3)
            #import pdb; pdb.set_trace()
            if True :
                ret_dict = lidar_values
                #if desc_from_logg == False:
                if encoder_chekp is None:
...
class GitModel(GitPreTrainedModel):
    ...
    def forward(
        ...
        lidar_values: Optional[dict] = None,
        input_mod: Optional[str] = None,
        encoder_chekp: Optional[str] = None,  # new
        ...
        ret_dict,projected_lidar_features = self.lidar_encoder(points,lidar_values,input_mod, encoder_chekp) # load
...
class GitForCausalLM(GitPreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.git(
            ...
            input_mod='frame_id',
            encoder_chekp = lidar_values['encoder_chekp'],
            points=points,
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
            past_key_values=past_key_values,
