...
class SSTInputLayer(nn.Module):
    ...
    def forward(self, input_dict):
        ...
        #print("warn drop voxel disable to work on cpu") 
...
class SPTBackbone(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        assert torch.all(voxel_coords[:, 1] == 0) # auto controle
        input_sp_tensor = spconv.SparseConvTensor(
            features=voxel_features, # torch.Size([49546, 128])
            indices=voxel_coords[:, [0, 2, 3]].contiguous().int(),  # (bs_idx, y_idx, x_idx) torch.Size([49546, 3])
            spatial_shape=self.sparse_shape, # array([248, 216])
            batch_size=batch_size #16
        )


        x = input_sp_tensor  # =  [shape=torch.Size([49546, 128])]
        x_hidden = []
        
        #print("inside SPTBackbone forward ")
        ...
        #print("before spatial ")

        
        spatial_features = []
        spatial_features_stride = []
        for i, src in enumerate(self.model_cfg.FEATURES_SOURCE): #3 0 x_conv1, 1 x_conv2, 2 x_conv3
            per_features = multi_scale_3d_features[src].dense() # torch.Size([16, 256, 62, 54])
            B, Y, X = per_features.shape[0], per_features.shape[-2], per_features.shape[-1] # 16, 62, 64
            #print(i, src, B, Y, X)
            #print("per_features_1" + str(per_features.std()))
            spatial_features.append(self.deblocks[i](per_features.view(B, -1, Y, X)))
            #print("per_features_2" + str(spatial_features[-1].std()))
            #print(self.deblocks[i][0].weight)
            # print("self.deblocks[i] ", self.deblocks[i])
            # print("(per_features.view(B, -1, Y, X)) ", np.shape(per_features.view(B, -1, Y, X)))
            # print("self.deblocks[i](per_features.view(B, -1, Y, X)) ", np.shape(self.deblocks[i](per_features.view(B, -1, Y, X))))
            
            spatial_features_stride.append(multi_scale_3d_strides[src] // self.model_cfg.FUSE_LAYER[src].UPSAMPLE_STRIDE)

        spatial_features_old = torch.cat(spatial_features, dim=1)
        ...
        # print("vox_coords" + str(voxel_coords))
        # print("vox_feature" + str(voxel_features.std()))
        # print("spatial_features_old" + str(spatial_features_old.std()))
        # print("spatial_features" + str(spatial_features.std()))
