...
def parse_config():
    ...
    parser.add_argument('--resume_from_checkpoint', type=bool, default=False, help='specify the config for training')
...
def make_compute_metrics(tokenizer, logger,rank,eval_set, sequence_path, d=None, closest={}):
    def compute_metrics(eval_preds):
        args, cfg = parse_config()
        #print("eval_preds.predictions ", eval_preds.predictions)
        # import pdb; pdb.set_trace()                    
        hit_at_1 = 0
        hit_at_10 = 0
        hit_at_1_clos = 0
        ...
        positions_database = eval_set.positions_database
        ##################################
        ...
        for beams, label in zip(eval_preds.predictions, eval_preds.label_ids):
            ...
            """
            if eval_set.do_self_eval and False :             
                filtered_rank_list = []            
                ...
                if rank == 0 :
                    if h1 :
                        logger.info("[o] id:" + str(label_id) + " n_id:" + str(rank_list[0]) + " pred:" +  ",".join(rank_list))
                    elif h10 :
                        logger.info("[~] id:" + str(label_id) + " n_id:" + str(rank_list[0]) + " pred:" +  ",".join(rank_list))
                    else :
                        logger.info("[ ] id:" + str(label_id) + " n_id:" + str(rank_list[0]) + " pred:" +  ",".join(rank_list))

                    if False :
                        logger.info("[ ] id:" + str(label_id_conv) + " n_id:" + str(rank_list_conv[0]) + " pred:" +  ",".join(rank_list_conv))
            
                
            else : 
            """
            if eval_set.labeltype == 'gps' :
                #label_id_gps = eval_set.gps2position(label_id)

                
                print("sequence_path  dict_gps_2_label_v2.json ", sequence_path)
                f = open(sequence_path + "/dict_gps_2_label_v2.json",) # gps xyxyxyxy
                data = json.load(f)
                f.close()

                query_id = data[label_id]
                answers_ids = [data[label_id] for label_id in rank_list]
                
                label_id_gps = positions_database[int(query_id)]
                #rank_list_gps_old = [eval_set.gps2position(x) for x in  answers_ids]
                rank_list_gps = [positions_database[int(x)] for x in  answers_ids]

                
                rank_list_dist = [str(round(math.dist(label_id_gps[0:2] , rank_list_gps[x][0:2]), 2)) for x in range(len(rank_list_gps))]
                rank_list_dist_filter = ['0' if float(x)>3 else '1' for x in rank_list_dist]   
                print("query_id", query_id, " answers_ids ", answers_ids, " label_id_gps ", label_id_gps, " rank_list_gps ", rank_list_gps, " rank_list_dist ", rank_list_dist )
                print("rank_list_dist_filter ", rank_list_dist_filter)

            elif eval_set.labeltype == 'hierarchical' :
                print("sequence_path  hierarchical.json ", sequence_path)
                f = open(sequence_path + "/hierarchical.json")
                data = json.load(f)
                f.close()
                ...
                print("sequence_path  hilbert.json ", sequence_path)
                f = open(sequence_path + "/hilbert.json",) 
                data = json.load(f)
                f.close()
            ...
            elif eval_set.labeltype == 'mixte' :
                print("sequence_path mixte.json ", sequence_path)
                f = open(sequence_path + "/mixte.json",) 
                data = json.load(f)
                f.close()
            ...
            else : #  eval_set.labeltype == 'label'
                label_id_gps = positions_database[int(label_id)]
                rank_list_gps = [positions_database[int(x)] for x in rank_list]
                rank_list_dist = [str(round(math.dist(label_id_gps[0:2] , rank_list_gps[x][0:2]), 2)) for x in range(len(rank_list_gps))]
                rank_list_dist_filter = ['0' if float(x)>3 else '1' for x in rank_list_dist]   
                
                print("query_id", label_id, " answers_ids ", rank_list, " label_id_gps ", label_id_gps, " rank_list_gps ", rank_list_gps, " rank_list_dist ", rank_list_dist )
                print("rank_list_dist_filter ", rank_list_dist_filter)
            ...
            if rank == 0 :                    
                if h1_clos :
                    logger.info("[o] id:" + str(label_id) + " n_id:" + str(rank_list[0]) + " pred dist:" +  ",".join(rank_list_dist))
                elif h10_clos :
                    logger.info("[~] id:" + str(label_id) + " n_id:" + str(rank_list[0]) + " pred dist:" +  ",".join(rank_list_dist))
                else :
                    logger.info("[ ] id:" + str(label_id) + " n_id:" + str(rank_list[0]) + " pred dist:" +  ",".join(rank_list_dist))
            #logger.info("       " + str(label_id) + " pred dist:" +  ",".join(rank_list_dist_filter))
        #import pdb; pdb.set_trace()
        #" , ".join([str(x) for x in rank_list])
                if False :
                    logger.info("[ ] id:" + str(label_id_conv) + " n_id:" + str(rank_list_conv[0]) + " pred:" +  ",".join(rank_list_conv))
        ...
        # with open("/tmp/out.json", 'w', encoding ='utf8') as json_file: 
        #     json.dump(dict_res, json_file, allow_nan=False)
        ...
        with open(save_file_name, 'a') as f:
            f.write(str(hit_at_1 / len(eval_preds.predictions)) + " " + str(hit_at_10 / len(eval_preds.predictions)) + "\n")
        f.close()
...
class DSITrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False): # 1
        ...
        loss = outputs.loss
        if return_outputs:
            return loss, [None, None]  # fake outputs :  loss, outputs
        return loss 
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            #seq_score = batch_beams_dict['sequences_scores'].reshape([self.per_device_eval_batch_size,nb_beam])
            seq_score = batch_beams_dict['sequences_scores'].reshape([-1,nb_beam]) # car full sequence n'est pas un multiple de 32
            ...
            batch_beams = batch_beams.reshape(inputs['input_ids'].shape[0], self.id_max_length, -1)

            if True :
                for ii in range(len(ids)) :
                    label_id = self.tokenizer.batch_decode(batch_beams.cpu()[ii], skip_special_tokens=True)

                    print("ids   :",ids[ii])
                    print("label :",vv[ii])
                    print("beam  :",str(label_id))
                    print("score :", seq_score[ii] )
                    print("----")

            #print(scores)

        #import pdb; pdb.set_trace()
        #return (None, batch_beams, inputs['lidar_values']['truth'])
        #print("inputs['labels'] ", inputs['labels'])
        #print("inputs['lidar_values']['truth'] ", inputs['lidar_values']['truth'])
...
class IndexingCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features):
        ...
        input_ids = [{'input_ids': x['input_ids']} for x in features]
        docids = [x['labels'] for x in features]
        #print("docids ", docids)
        #print("gtruthids ", gtruthids)

        inputs = super().__call__(input_ids)
        #print("IndexingCollator inputs ", inputs)
        ## We are doing our own stuff with input_ids and replacing the heritated init
        input_ids = torch.vstack([x['input_ids'] for x in features])
        labels = torch.vstack([x['input_ids'] for x in features])
        ids = [x['id'] for x in features]
        #labels = torch.vstack([x['labels'] for x in features])
        atm = torch.vstack([x['attention_mask'] for x in features])
        #print("labels ", labels)
        #gtruthids = [int(i) for i in gtruthids]
        # closest = torch.tensor([int(i) for i in gtruthids])
        #print("[x['truth'] for x in features] ", gtruthids)
        #closest = torch.vstack([ torch.tensor([int(i) for i in x['truth']]) for x in features])
        atm[input_ids==self.tokenizer.eos_token_id] = 0
        input_ids[input_ids==self.tokenizer.eos_token_id] = self.tokenizer.pad_token_id
        inputs['attention_mask'] = atm.to(device=device)
        inputs['input_ids'] = input_ids.to(device=device)
        inputs['labels'] = labels.to(device=device)
        inputs['ids'] = ids
        inputs['pixel_values'] = torch.cat([x['pixel_values'] for x in features], dim=0).to(device=device)
        ll_keys = ['labels','input_ids','pixel_values']
        for it in features : 
            for kk in ll_keys :
                del it[kk]
        lidar_val = {}
        lidar_val['batch_size'] = self.batch_size
        ll_keys = list(features[0].keys())
        for key in ll_keys :
            val = [x[key] for x in features]
            if key in ['points']:
                coors = []
                for i, coor in enumerate(val):
                    coor_pad = np.pad(coor, ((0, 0), (1, 0)), mode='constant', constant_values=i)
                    coors.append(coor_pad)
                lidar_val[key] = torch.from_numpy(np.concatenate(coors, axis=0)).float().to(device=device)
            else:
                lidar_val[key] = np.stack(val, axis=0)
        inputs['lidar_values'] = lidar_val

        if device == "cuda" :
            #print("inputs ", inputs, inputs['lidar_values'])
...
def main():
    ...
    do_generative_test = True #never used
...
#    import pdb; pdb.set_trace()    
    ...
    if dataset_train_len > 0 : #if 64 > 0
        train_len = int(dataset_train_len)
    else :
        train_len = len(train_set)

    if dataset_eval_len > 0 : #if 64 > 0
        eval_len = int(dataset_eval_len)
    else :
    ...
    do_overfit = True
    if do_overfit : 
        eval_subset = train_subset
    else :
        eval_subset = torch.utils.data.Subset(eval_set,range(0,train_len))
   
        
    ### ========= build Models ===========
    # models paths
    WORK_PATH = os.getenv('WORK')
    model_git_path = WORK_PATH + "/datas/transformers/git-base-coco"
    model_blip2_path = WORK_PATH + "/datas/transformers/blip2-opt-2.7b"
    model_bert_path = WORK_PATH + "/datas/transformers/bert-base-uncased"

    
    ### === MAE ===
    num_class=len(cfg.CLASS_NAMES)        
    model_mae = build_network(model_cfg=cfg.MODEL, num_class=num_class, dataset=train_set, logger=logger)
    if args.sync_bn:
        model_mae = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_mae)
    if device == "cuda" :
        model_mae.cuda()
    optimizer = build_optimizer(model_mae, cfg.OPTIMIZATION)
    ...
    if args.pretrained_model is not None:
        print('model_mae.load_params_from_file ',args.pretrained_model)
        model_mae.load_params_from_file(filename=args.pretrained_model, to_cpu=dist_train, logger=logger)
    if args.ckpt is not None:
        print('model_mae.load_params_with_optimizer ',args.ckpt)
        it, start_epoch = model_mae.load_params_with_optimizer(args.ckpt, to_cpu=dist_train, optimizer=optimizer, logger=logger)
        last_epoch = start_epoch + 1
    else:
        ckpt_list = glob.glob(str(ckpt_dir / '*checkpoint_epoch_*.pth'))
        if len(ckpt_list) > 0:
            ckpt_list.sort(key=os.path.getmtime)
            it, start_epoch = model_mae.load_params_with_optimizer(
    ...
    if model_name == "git" or  (args.git_checkpoint is not None) :
        if args.git_checkpoint is not None :
            model_dsi_path = args.git_checkpoint
        model_dsi_path = model_git_path
        config = AutoConfig.from_pretrained(model_dsi_path)

        #import pdb; pdb.set_trace()
        #config.vision_config.image_size=250
        #model_dsi = GitForCausalLM.from_pretrained(model_dsi_path).to(device=device)
        model_dsi = GitForCausalLM(config).to(device=device) #
        #import pdb; pdb.set_trace()
        tokenizer = AutoTokenizer.from_pretrained(model_dsi_path)

        model_dsi.set_lidar_model(model_mae,model_sop,do_use_sop,train_set.root_path)
        if args.git_checkpoint is not None : 
    ...
    # BLIP2
    if model_name == "blip2" :
        model_dsi_path = model_blip2_path # blip2-opt-2.7b"
        config = AutoConfig.from_pretrained(model_dsi_path)
        #model_dsi = Blip2ModelQuerryLearning.from_pretrained(model_blip2_path, device_map="auto",return_dict=True).type(torch.float32)
        #gptq_config = GPTQConfig(bits=8, disable_exllama=False)
        #config.qformer_config.encoder_hidden_size = 768
        model_dsi = Blip2ModelQuerryLearning(config=config).to(device=device).type(torch.float32)
        tokenizer = AutoTokenizer.from_pretrained(model_dsi_path)
        model_bert = BertGenerationDecoder.from_pretrained(model_bert_path,is_decoder=True).to(model_dsi.device)
        tokenizer_bert = AutoTokenizer.from_pretrained(model_bert_path)
        tokenizer = tokenizer_bert
        model_dsi.logger = logger
        ...
        if args.git_checkpoint is not None :
            model_dsi.set_input_embeddings(model_bert.bert.embeddings,input_embeddings)
            model_dsi.set_output_embeddings(output_embeddings)
            model_dsi.lidar_model.set_lidar_encoder(model_mae,lidar_projection,bt_norm)
        else :
            #model_dsi.lidar_model.set_lidar_encoder(model_mae)
            model_dsi.lidar_model.set_lidar_model(model_mae,model_sop,do_use_sop,train_set.root_path)            
            #model_dsi.lidar_model.set_lidar_model(model_mae)
            model_dsi.set_input_embeddings(model_bert.bert.embeddings,None)
        #model_dsi.set_output_embeddings(model_bert.lm_head)
        # model_dsi.lidar_model.bt_norm = model_dsi.lidar_model.lidar_encoder.bt_norm
        # model_dsi.lidar_model.lidar_projection = model_dsi.lidar_model.lidar_encoder.lidar_projection
    ...
    n_subset = [int(x) for x in range(len(train_subset))] # = [0,..63]
    n_set = [int(x) for x in range(len(train_set))] # = [0,..4540] # tt le vocab
    #n_set = [int(x) for x in range(len(eval_set))] # = [0,..4540] # tt le vocab
    #print("lla ", lla) 
    lid = []
    LIK = []
    mid = {}
    #MIK = {}
    #rrg = 10
    for ii in n_subset : lid.append(train_set.get_label(ii))
        #mid[ii] = []
        #MIK[ii] = []
        #for jj in range(-rrg,rrg) :
            #mid[ii].append(ii + jj)
            #MIK[ii].append(tokenizer(ii+jj,padding="max_length",max_length=ID_MAX_LENGTH).input_ids)        
    for ii in lid : LIK.append(tokenizer(ii,padding="max_length",max_length=ID_MAX_LENGTH).input_ids)
    #for ii in range(len(LIK)) :
    #    print("lid:" + str(lid[ii]) + " lik:" + str(LIK[ii]))

 # tokenizer('15008883',padding="max_length",max_length=ID_MAX_LENGTH).input_ids) 
    # restrict code
    # import pdb; pdb.set_trace()
    ...
    ### ====== Freezing Model =========
    ## Freeze network
    model_mae.freeze(model_mae.model_cfg.FREEZE_LAYERS)
    if True : 
    ...
    ###  ========= Weight Balancing ======== 
    #valid_ids = {} # toujours vide
    valid_labes = []
    #acc = 0 # jamais utilisé
    for ii in range(0,len(train_set.kitti_infos)) :
        llab = train_set.get_label(ii) # ex '4540'
        valid_labes.append(llab)
        tokens = tokenizer(llab, padding="max_length",return_tensors="pt",truncation='only_first',max_length=ID_MAX_LENGTH).input_ids

    if args.weighted_crossentropy : # False
        flat_tk = np.array(LIK).flatten()
        ctt= Counter(flat_tk)
        total_samples = len(flat_tk)
        num_classes = len(ctt.keys())
        weights = torch.zeros(len(tokenizer.get_vocab()),dtype=torch.float16)
        for kk in ctt.keys() :
            weights[kk] = total_samples / (ctt[kk] * num_classes)
        model_dsi.set_weights(weights)
        
    model_dsi.set_vocab(INT_TOKEN_IDS) 
    print("Weight Balancing ")
    ...
    print("restrict_decode_vocab ", restrict_decode_vocab)
    #import pdb; pdb.set_trace()
        # Huggin face trainer
    ...
    if  eval_set.labeltype == 'gps' :         
        checkpoint_dir = "dsi_iter_256_gps_10dg"
        #checkpoint_dir = "dsi_iter_256_gps_tmp"
    elif eval_set.labeltype == 'hierarchical' :
        checkpoint_dir = "dsi_hierarchical"
    elif eval_set.labeltype == 'hilbert' :
        checkpoint_dir = "dsi_hil"    
    else :
        checkpoint_dir = "dsi_iter_256_label"
    ...
    if do_train :
        ...
        for query_idx in range(train_len): 
            if query_idx% 5 != 0:
                train_indices.append(query_idx) # training set 
            else:
                if query_idx% 10 != 0:
                    val_indices.append(query_idx)  # validation set 
                else:
                    eval_indices.append(query_idx)  # test set 
        ...
        previous_model_path = "/lustre/fswork/projects/rech/dki/ujo91el/checkpoints/git_gps_git_gps_seq_00_80_20_contrast_quad/checkpoint-9600/" 
        ...
        trainer = DSITrainer(
                ...
                compute_metrics=make_compute_metrics(tokenizer, logger,args.local_rank, eval_set, closest=closest, sequence_path=sequence_path),
                restrict_decode_vocab=restrict_decode_vocab,
                LIK=LIK,
                #callbacks=[EarlyStoppingCallback(metric_name="eval_Hits@1", threshold=0.99)],  # Custom callback
                id_max_length=ID_MAX_LENGTH
            ) 
#compute_metrics=make_compute_metrics(tokenizer, valid_labes, logger,args.local_rank, eval_set, closest=closest, sequence_path=sequence_path),
        if not os.path.isdir(previous_model_path) :
            print("train raw : " + cur_model_path)                
            trainer.train()
            is_training = True
        else :
    ...
    #do_eval = args.do_log3dnet_eval
    if do_eval  : 
        print("start eval")
        if do_eval_partial :
            model_path = checkpoint_dir +  "/" + str(880) + "/pytorch_model.bin"
            print("model_path:" + model_path)
            if os.path.exists(model_path) :
                print("LOAD NEW PATH:" + model_path) 
                state_dict = torch.load(model_path)
                model_dsi.load_state_dict(state_dict, False)
                del state_dict
                model_dsi.eval()
        eval_log3dnet(model_dsi,eval_subset,eval_set,data_collator,tokenizer,cfg,checkpoint_dir, do_eval_partial)
