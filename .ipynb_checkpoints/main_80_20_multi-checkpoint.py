...
def setup_distributed():
    """ Initialize distributed backend with proper checks. """
    if "RANK" not in os.environ or "WORLD_SIZE" not in os.environ:
        print("Running in single-process mode (not distributed).")
        return -1  # Single-process mode
...
# #############################################################################
def make_compute_metrics(tokenizer, logger, rank, positions_database, label_mapping):
...
class IndexingCollator(DataCollatorWithPadding):
    ...
    def _prepare_lidar_values(self, features, device):
        ...
        for key, val in {k: [x[k] for x in features] for k in features[0].keys()}.items():
            if key == 'points':
                # Use a list comprehension to pad and concatenate in one step
                lidar_val[key] = torch.from_numpy(
                    np.concatenate(
                        [np.pad(coor, ((0, 0), (1, 0)), mode='constant', constant_values=i) for i, coor in enumerate(val)],
                        axis=0
                    )
                ).float().to(device=device)
            else :
                # Directly store string data without tensor conversion
...
#    import pdb; pdb.set_trace()    
    ...
    # Only use DDP if running in multi-GPU mode
    ...
    if do_train:
        ...
        positions_database = train_set.positions_database
        ...
        trainer = DSITrainer(
                ...
                compute_metrics=make_compute_metrics(tokenizer, logger, args.local_rank, positions_database, label_mapping),
    ...
    if do_eval  : 
        del train_set
        print("start eval")
        eval_log3dnet(model_dsi,eval_subset,eval_set,data_collator,tokenizer,cfg, checkpoint_dir)
